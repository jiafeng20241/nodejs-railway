...
// 修复：使用jsdelivr全球CDN下载Xray，100%可访问
async function downloadAndExtractXray(maxRetries = 3) {
  const arch = getArch();
  const osType = "linux";
  const version = "v26.2.6"; // 最新稳定版本
  // 使用jsdelivr全球CDN镜像，Railway必能访问
  const downloadUrls = [
    `https://cdn.jsdelivr.net/gh/XTLS/Xray-core@${version}/releases/download/${version}/Xray-${osType}-${arch}.zip`,
    `https://fastly.jsdelivr.net/gh/XTLS/Xray-core@${version}/releases/download/${version}/Xray-${osType}-${arch}.zip` // 备用节点
  ];
  const zipPath = path.join(CONFIG.FILE_PATH, `Xray-${osType}-${arch}.zip`);
  ...
  for (const url of downloadUrls) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        logger.info(`Downloading Xray from ${url} (attempt ${attempt}/${maxRetries})...`);
  ...
  throw new Error(
    `Failed to download Xray after all attempts: ${lastError.message}`
  );
}
...
async function boot() {
  ...
  const urls = {
    argo: "https://cdn.jsdelivr.net/gh/cloudflare/cloudflared@latest/releases/latest/download/cloudflared-linux-amd64",
  };
